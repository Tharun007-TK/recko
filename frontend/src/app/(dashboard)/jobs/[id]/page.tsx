...
import { JobStatusPoller } from "./_components/JobStatusPoller";
import { getReportDownloadUrl } from "@/actions/jobs";
...
export default async function JobDetailPage({
  ...
  if (job.status === "completed" && reportFile) {
    reportUrl = await getReportDownloadUrl(reportFile.storage_path);
  }
    ...
    <div className="space-y-6">
      ...
      >
        <div className="flex gap-2">
          {job.status === "completed" && reportUrl && (
            <Button asChild>
              <a href={reportUrl} download="reconciliation_report.xlsx">
      ...
      <JobStatusPoller jobId={job.id} status={job.status} />
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent className="text-sm">
            {Array.isArray(job.mapping_profiles) ? job.mapping_profiles[0]?.name : (job.mapping_profiles as any)?.name || "None"}
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="text-sm">
            {Array.isArray(job.rule_profiles) ? job.rule_profiles[0]?.name : (job.rule_profiles as any)?.name || "None"}
          </CardContent>
        </Card>
      </div>
